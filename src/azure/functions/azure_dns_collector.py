"""
Azure DNS Analytics Log Collector — Azure Function

Collects Azure DNS Analytics logs routed from Azure Monitor diagnostic
settings via Event Hub and stores normalised events in Azure Blob Storage
for data lake analysis.

Delivery Method:
  Azure Monitor diagnostic settings route DNS Analytics logs to an Event Hub.
  This Azure Function uses an Event Hub trigger to process incoming batches.

Data Lake Output:
  ``{CONTAINER}/azure_dns/raw/YYYY/MM/DD/HH/<partition>.json``
...
from parsers.azure_dns import AzureDNSParser  # noqa: E402

_parser = AzureDNSParser()
...
def _decode_event_hub_messages(messages: List[Any]) -> List[Dict[str, Any]]:
    """Decode Event Hub messages into DNS log entry dicts.
    ...
    Args:
        messages: List of Event Hub message bodies (bytes or str or dict).

    Returns:
        List of parsed DNS log entry dicts.
    """
    ...
    for message in messages:
        try:
            ...
            if isinstance(data, dict):
                # Azure Monitor diagnostic logs often wrap entries in "records"
...
def _decode_http_request(request_json: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    Supports direct entry submission and records array format.

    Args:
        request_json: Request JSON body.

    Returns:
        List of DNS log entry dicts.
    """
    entries = []
    ...
    # Single entry (body itself looks like a DNS log)
    if not entries and ("properties" in request_json or "category" in request_json):
...
def _parse_dns_events(raw_events: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Parse raw Azure DNS Analytics log entries.

    Args:
        raw_events: List of raw DNS log entry dicts.
...
def _write_to_blob(
    ...
    blob_path = f"azure_dns/raw/{year}/{month}/{day}/{hour}/dns_{partition_id}.json"
...
def event_hub_trigger(messages: List[Any]) -> Dict[str, Any]:
    """Event Hub triggered function for Azure DNS Analytics logs.

    Processes batches of DNS log entries received from Azure Monitor
...
def http_trigger(body: Dict[str, Any]) -> Dict[str, Any]:
    """HTTP trigger for on-demand processing of DNS Analytics logs.
